...
replacement = r'''    function renderArticleGallery(article, copy) {
      const galleryImages =
...
gallery_css = '''    .article-gallery-track {
      display: block;
      overflow: hidden;
      width: 100%;
    }
    ...
    .article-hero-image {
      ...
      min-height: 560px;
      background:
        radial-gradient(circle at 25% 25%, rgba(255,255,255,.13), transparent 32%),
...
'''
css = css[:start] + gallery_css + css[end:]

old_mobile = '''      .article-hero-image,
      .feature-image {
        height: 380px;
        min-height: 380px;
      }'''
new_mobile = '''      .feature-image {
        height: 380px;
        min-height: 380px;
      }
...
if 'css/style.css?v=gallery26' not in index or 'js/app.js?v=gallery26' not in index:
    raise SystemExit('Expected gallery26 cache keys not found')
index = index.replace('css/style.css?v=gallery26', 'css/style.css?v=gallery27', 1)
index = index.replace('js/app.js?v=gallery26', 'js/app.js?v=gallery27', 1)
